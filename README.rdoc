...
can either set the $redis global variable:
    ...
    @list  = Redis::Type::List.new('mylist')

Or you can pass the Redis handle into the new method for each type:

    redis  = Redis.new(:host => 'localhost', :port => 6379)
    @list  = Redis::Type::List.new('mylist', redis)
...
Create a new counter. The +counter_name+ is the key stored in Redis.
    ...
    @counter = Redis::Type::Counter.new('counter_name')
    @counter.increment
    @counter.decrement
    puts @counter.value
...
A convenience class that wraps the pattern of {using +setnx+ to perform locking}[http://code.google.com/p/redis/wiki/SetnxCommand].
    ...
    require 'redis/lock'
    @lock = Redis::Type::Lock.new('image_resizing', :expiration => 15, :timeout => 0.1)
    @lock.lock do
      # do work
    end
...
Simple values are easy as well:
    ...
    @value = Redis::Type::Value.new('value_name')
    @value.value = 'a'
    @value.delete
...
Complex data is no problem with :marshal => true:
    ...
    @newest  = Redis::Type::Value.new('newest_account', :marshal => true)
...
Lists work just like Ruby arrays:
    ...
    @list = Redis::Type::List.new('list_name')
    @list << 'a'
    @list << 'b'
...
You can bound the size of the list to only hold N elements like so:
    ...
    @list = Redis::Type::List.new('list_name', redis_handle, :maxlength => 10)

Complex data types are no problem with :marshal => true:

    @list = Redis::Type::List.new('list_name', :marshal => true)
...
a few Redis-specific additions.
    ...
    @hash = Redis::Type::Hash.new('hash_name')
    @hash['a'] = 1
    @hash['b'] = 2
    @hash.each do |k,v|
      puts "#{k} = #{v}"
...
Sets work like the Ruby {Set}[http://ruby-doc.org/core/classes/Set.html] class:
    ...
    @set = Redis::Type::Set.new('set_name')
    @set << 'a'
    @set << 'b'
    @set << 'a'  # dup ignored
    @set.member? 'c'      # false
...
You can perform Redis intersections/unions/diffs easily:
    ...
    @set1 = Redis::Type::Set.new('set1')
    @set2 = Redis::Type::Set.new('set2')
    @set3 = Redis::Type::Set.new('set3')
...
And use complex data types too, with :marshal => true:
    ...
    @set1 = Redis::Type::Set.new('set1', :marshal => true)
    @set2 = Redis::Type::Set.new('set2', :marshal => true)
...
a Hash and an Array.  You assign like a Hash, but retrieve like an Array:
    ...
    @sorted_set = Redis::Type::SortedSet.new('number_of_posts')
    @sorted_set['Nate']  = 15
    @sorted_set['Peter'] = 75
    @sorted_set['Jeff']  = 24
